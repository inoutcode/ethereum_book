...
* 提示你選擇密碼時：強化它，備份並不共享。如果你沒有密碼管理器，請將其寫下並存放在鎖定的抽屜或保險櫃中。只要你擁有帳戶的“keystore”文件，就可以使用此密碼。

* 當系統提示你備份密鑰或助記詞時，請使用筆和紙進行物理備份。不要把這個任務放在“以後”，你會忘記。這些在你丟失了系統中保存的所有數據時使用。

* 不要在數字文檔，數字照片，屏幕截圖，在線驅動器，加密的PDF等中存儲密鑰材料（加密或不加密），不要臨時湊合的安全性。使用密碼管理器或筆和紙。
...
別擔心！如果你選擇一個錢包而不喜歡它的工作方式，那麼你可以很容易地更換錢包。你只需進行一項交易，即將資金從舊錢包發送到新錢包，或者通過導出和導入私鑰來移動密鑰。

首先，我們將選擇三種不同類型的錢包作為整本書的示例：移動錢包，桌面錢包和基於網絡的錢包。我們選擇了這三款錢包，因為它們代表了廣泛的複雜性和功能。然而，選擇這些錢包並不是對其質量或安全性的認可。他們只是示範和測試。
...
MyEtherWallet (MEW):: MyEtherWallet是一款基於網絡的錢包，可在任何瀏覽器中運行。它具有多個複雜的功能，我們將在許多示例中探討這些功能。

Emerald Wallet:: Emerald錢包設計用於以太坊經典區塊鏈，但與其他以太坊區塊鏈兼容。它是一款開源桌面應用程序，適用於Windows，Mac和Linux。Emerald錢包可以運行一個完整的節點或連接到一個公共的遠程節點，工作在“輕量”模式下。它還有一個配套工具來在命令行中執行所有操作。
...
確認你正在查看正確的擴展程序後，請點擊“添加到Chrome”進行安裝。
...
[TIP]
====
在紙上備份助記符（12個字），兩次。將兩份紙張備份存放在兩個單獨的安全位置，例如防火安全櫃，鎖定的抽屜或保險箱。將紙質備份視為你在Ethereum錢包中存儲的相同價值的現金。任何能夠訪問這些文字的人都可以訪問並竊取你的資金。
====

一旦確認你已安全存儲助記符，MetaMask將顯示你的以太坊帳戶詳細信息：
...
Ropsten Test Network:: 以太坊公開測試區塊鏈和網絡，使用工作證明共識（挖礦）。在這個網絡上的ETH沒有價值。Ropsten的問題在於攻擊者鑄造了數以萬計的區塊，產生巨大的重組並將燃氣極限推到9B。當時需要一個新的公共測試網，但之後（2017年3月25日）Ropsten也復活了！

Kovan Test Network:: 以太坊公開測試區塊鏈和網絡，使用“Aura”協議進行權威證明（Proof-of-Authority）共識（聯合簽名）。在這個網絡上的ETH沒有價值。該測試網絡僅由“Parity”支持。其他以太坊客戶使用稍後提出的"Clique"協議作為權威證明。

Rinkeby Test Network:: 以太坊公開測試區塊鏈和網絡，使用“Clique”協議進行權威證明共識（聯合簽名）。在這個網絡上的ETH沒有價值。

Localhost 8545:: 連接到與瀏覽器在同一臺計算機上運行的節點。該節點可以是任何公共區塊鏈（主要或測試網絡）或私人測試網絡的一部分（參見<<ganache>>）。
...
[TIP]
====
你的MetaMask錢包在連接的所有網絡上使用相同的私鑰和以太坊地址。但是，每個以太坊網絡上的以太坊地址餘額將有所不同。例如，你的密鑰可以控制Ropsten上的以太和合約，但不能控制主網上的。
====
...
將MetaMask切換到_Ropsten測試網絡_。然後點擊“Buy”，然後點擊“Ropsten Test Faucet”。MetaMask將打開一個新的網頁：

[[metamask_ropsten_faucet]]
.MetaMask Ropsten Test Faucet
image::images/metamask_ropsten_faucet.png["MetaMask Ropsten Test Faucet"]

你可能會注意到該網頁已經包含你的MetaMask錢包的以太坊地址。MetaMask集成了支持以太坊的網頁（ 參見 <<dapps>>）與你的MetaMask錢包整合在一起。MetaMask可以在網頁上“查看”以太坊地址，例如，你可以向顯示以太坊地址的網上商店發送付款。如果網頁請求，MetaMask也可以使用自己的錢包地址填入網頁，作為收件人地址。在此頁面中，faucet應用程序要求MetaMask提供一個錢包地址以發送測試以太網。

按綠色"request 1 ether from faucet"按鈕。你會看到一個交易ID出現在頁面的下方。faucet應用程序創建了一個交易 - 付款給你。交易ID如下所示：
...
嘗試訪問該鏈接，或將交易哈希值輸入到 +ropsten.etherscan.io+ 網站中，親自查看。

[[sending_eth_MetaMask]]
=== 使用MetaMask發送ether

一旦我們從Ropsten Test Faucet接收到我們的第一個測試ether，我們將試著發送一些ether回到faucet。正如你在Ropsten Test Faucet頁面上看到的那樣，你可以選擇“donate”1個ETH。這個選項是可用的，所以一旦你完成了測試，你可以返回剩餘的測試ether，以便其他人可以使用它。儘管測試ether沒有價值，但有些人囤積測試ether，使其他人難以使用測試網絡。囤積測試ether令人不悅！
...
[TIP]
====
測試網絡也需要費用。如果沒有費用，測試網絡的行為將與主網絡不同，從而使其成為不適當的測試平臺。費用還可以保護測試網絡免受拒絕服務攻擊和構造不良的合約（如無限循環），就像保護主網絡一樣。
====
...
我們已經創建了一個錢包，並且我們已經發送並接收了ether。到目前為止，我們已經將以太坊視為一種加密貨幣。但是以太坊代表了更多。事實上，加密貨幣功能是服務於以太坊作為世界計算機的功能; 一個去中心化的智能合約平臺。以太旨在用於支付運行的 _smart contracts_，這是在稱為_Ethereum虛擬機（EVM）_的模擬計算機上運行的計算機程序。

EVM是一個全球性的單例，這意味著它的運作方式就好像它是一個全球性的單實例計算機，無處不在。以太坊網絡上的每個節點運行EVM的本地副本以驗證合約執行，而以太坊區塊鏈記錄此世界計算機在處理交易和智能合約時變化的 _狀態_。

[[EOA_contracts]]
=== 外部所有賬戶（EOAs）和合約

我們在MetaMask錢包中創建的賬戶類型稱為 _Externally Owned Account（EOA）_ 。外部所有賬戶是那些擁有私人密鑰的賬戶，它控制對資金或合約的訪問。現在，你可能猜測還有另一種帳戶，_合約_帳戶。合約賬戶由以太坊區塊鏈記錄，由EVM執行的軟件程序的邏輯所擁有（和控制）。

將來，所有以太坊錢包可能會作為以太坊合約運行，模糊了外部所有賬戶和合約賬戶之間的區別。但是永遠保持的重要區別在於：人們通過EOA做出決定，而軟件通過合約做出決定。
...
以太坊有許多不同的高級語言，所有這些語言都可用於編寫合約並生成EVM字節碼。你可以閱讀 <<high_level_languages>>  中許多最成功和有趣的內容。一種智能合約編程的主要高級語言：Solidity。本書的合著者Gavin Wood創建了Solidity，已經成為以太坊及以太坊外最廣泛使用的語言。我們將使用Solidity編寫我們的第一份合約。

作為我們的第一個例子，我們將編寫一個控制_faucet_的合約。我們已經使用了faucent在Ropsten測試網絡上獲得測試ether。faucet是一件相對簡單的事情：它給任何地址發放ether，可以定期補充。你可以將faucet實現為由人類（或網絡服務器）控制的錢包，但我們將編寫一個實現faucet的Solidity合約：
...
註釋用於人類閱讀，不包含在可執行的EVM字節碼中。我們通常將註釋放在我們試圖解釋的代碼之前，有時在同一行上。評論從兩個正斜槓 +//+ 開始。從斜線和直到該行結束的所有內容都被視為空白行並被忽略。
...
該行聲明瞭一個合約對象，類似於JavaScript，Java或Cpass:[++]等其他面嚮對象語言中的 +class+ 聲明。合約的定義包含了大括號中的所有行 pass:[{}]，它定義了 +範圍 +，就像在其他許多編程語言中使用花括號一樣。
...
+withdraw+方法的第一部分設置了取款限制。它使用內置的Solidity函數 +require+ 來測試前提條件，即 +withdraw_amount+ 小於或等於100000000000000000 +wei+，它是ether的基本單位（參見 <<ether_denominations>>），等於0.1 ether。如果使用 +withdraw_amount+ 大於該數量調用 +withdraw+ 函數，則此處的 +require+ 函數將導致合約執行停止並失敗，並顯示_異常_。

合約的這部分是我們faucet的主要邏輯。它通過設定提款限額來控制合約的資金流出。這是一個非常簡單的控制，但可以讓你看到可編程區塊鏈的強大功能：去中心化控制貨幣的軟件。
...
這裡發生了一些有趣的事情。+msg+ 對象是所有合約可以訪問的輸入之一。它代表觸發執行此合約的交易。屬性 +sender+ 是交易的發件人地址。函數 +transfer+ 是一個內置函數，它將ether從合約傳遞到調用它的地址。從後往前讀，表示 +transfer+ 到觸發此合約執行的 +msg+ 的 +sender+。+transfer+ 函數將一個金額作為唯一的參數。我們傳遞之前聲明為 +withdraw+ 方法的參數的 +withdraw_amount+ 值。
...
此函數是所謂的_“fallback”_或_default_函數，如果合約的交易沒有命名合約中任何已聲明的功能或任何功能，或者不包含數據，則觸發此函數。合約可以有一個這樣的默認功能（沒有名字），它通常是接收ether的那個。這就是為什麼它被定義為 +public+ 和 +payable+ 函數，這意味著它可以接受合約中的ether。除了大括號中的空白定義 pass:[{}] 所指示的以外，它不會執行任何操作。如果我們進行一次向這個合約地址發送ether的交易，就好像它是一個錢包一樣，該函數將處理它。

在我們的默認函數下面是最後一個關閉花括號，它關閉了合約 +faucet+ 的定義。就是這樣！

[[compile_faucet_contract]]
=== 編譯faucet合約

現在我們已經有了我們的第一個示例合約，我們需要使用Solidity編譯器將Solidity代碼轉換為EVM字節代碼，以便它可以由EVM執行。

Solidity編譯器是獨立的可執行文件，作為不同框架的一部分，也捆綁在一個_Integrated Development Environment（IDE）_中。為了簡單起見，我們將使用一種更流行的IDE，稱為Remix。
...
現在，點擊左側工具欄中的圓形加號，添加一個新選項卡，命名新文件 +Faucet.sol+：
...
現在我們已將 +Faucet.sol+ 合約加載到Remix IDE中，IDE將自動編譯代碼。如果一切順利，你會看到一個綠色的放開，右邊出現一個帶有“faucet”的綠色方塊，在Compile選項卡下，確認編譯成功：
...
你是不是很高興使用像Solidity這樣的高級語言，而不是直接在EVM字節碼中編程？我也是！
...
首先，切換到“Run”選項卡，並在“Environment”下拉列表框中選擇“Injected Web3”。這將Remix IDE連接到MetaMask錢包，並通過MetaMask連接到Ropsten測試網絡。一旦你這樣做，你可以在Evironment下看到“Ropsten”。另外，在Account選擇框中，它顯示你的錢包的地址：
...
讓我們回顧一下我們迄今為止學到的東西：以太坊合約是控制貨幣的程序，運行在名為EVM的虛擬機內。它們是由一個特殊的交易創建的，該交易提交它們的字節碼以記錄在區塊鏈中。一旦他們在區塊鏈上創建，他們就擁有一個以太坊地址，就像錢包一樣。只要有人將交易發送到合約地址，它就會導致合約在EVM中運行，並將交易作為其輸入。發送到合約地址的交易可能包含以太網或數據或兩者都有。如果它們含有ether，則將其“存入”合約餘額。如果它們包含數據，則數據可以在合約中指定一個命名函數並調用它，並將參數傳遞給該函數。
...
還記得我們的 +Faucet.sol+ 代碼中的未命名默認公共付費功能？它看起來像這樣：
...
當你將交易發送到合約地址時，沒有指定要調用哪個函數的數據，它將調用默認函數。由於我們將它聲明為+payable+，因此它接受1 ether並存入合約賬戶餘額中。你的交易導致合約在EVM中運行，更新其餘額。我們資助了我們的faucet！
...
提款的交易在哪裡？合約的地址歷史記錄頁面中出現了一個名為“內部交易”的新選項卡。由於0.1 ether傳輸源於合約代碼，因此它是一個內部交易（也稱為_message_）。點擊“內部交易”標籤查看：
...
這個“內部交易”是由合約在這行代碼中發送的（+Faucet.sol+ 的 +withdraw+ 方法）
...
本章中，我們使用MetaMask創建了一個錢包，並且使用Ropsten測試網絡上的一個faucet為它充值。我們收到了發送到錢包以太坊地址的ether。然後我們把ether發送到faucet的以太坊地址。
...
可能看起來不多，但我們剛剛成功地與控制去中心化世界計算機上資金的軟件進行了交互。
