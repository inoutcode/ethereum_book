...
我們已經創建了一個錢包，並且我們已經發送並接收了ether。到目前為止，我們已經將以太坊視為一種密碼貨幣。但是以太坊代表了更多。事實上，密碼貨幣功能是服務於以太坊作為世界計算機的功能; 一個去中心化的智能合約平臺。以太旨在用於支付運行的 _smart contracts_，這是在稱為_Ethereum虛擬機（EVM）_的模擬計算機上運行的計算機程序。
