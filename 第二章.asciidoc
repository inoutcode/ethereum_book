...
* 不要在數字文檔，數字照片，屏幕截圖，在線驅動器，加密的PDF等中儲存密鑰材料（加密或不加密），不要臨時湊合的安全性。使用密碼管理器或筆和紙。
...
該行聲明瞭一個合約對象，類似於JavaScript，Java或Cpass:[++]等其他面嚮對象語言中的 +class+ 聲明。合約的定義包含了大括號中的所有行 pass:[{}]，它定義了 +範圍 +，就像在其他許多程式語言中使用花括號一樣。
