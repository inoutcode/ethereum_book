...
我們在 <<intro>> 中發現，以太坊有兩種不同類型的賬戶：外部所有賬戶（EOAs）和合約賬戶。EOAs由以太坊以外的軟件（如錢包應用程序）控制。合約帳戶由在以太坊虛擬機（EVM）內運行的軟件控制。兩種類型的帳戶都通過以太坊地址標識。在本節中，我們將討論第二種類型，合約賬戶和控制它們的軟件：智能合約。
...
不可變的：一旦部署，智能合約的代碼不能改變。與傳統軟件不同，修改智能合約的唯一方法是部署新實例。
...
交易是 _原子性的_ _atomic_，無論他們調用多少合約或這些合約在被調用時執行的是什麼。交易完全執行，僅在交易成功終止時記錄全局狀態（合約，帳戶等）的任何更改。成功終止意味著程序執行時沒有錯誤並且達到執行結束。如果交易由於錯誤而失敗，則其所有效果（狀態變化）都會“回滾”，就好像交易從未運行一樣。失敗的交易仍存儲在區塊鏈中，並從原始賬戶扣除gas成本，但對合約或賬戶狀態沒有其他影響。
...
雖然任何高級語言都可以用來編寫智能合約，但這是一項非常繁瑣的工作。智能合約在高度約束和簡約的執行環境（EVM）中運行，幾乎所有通常的用戶界面，操作系統界面和硬件界面都是缺失的。從頭開始構建一個簡約的智能合約語言要比限制通用語言並使其適用於編寫智能合約更容易。因此，為編程智能合約出現了一些專用語言。以太坊有幾種這樣的語言，以及產生EVM可執行字節碼所需的編譯器。

一般來說，編程語言可以分為兩種廣泛的編程範式：分別是聲明式和命令式，也分別稱為“函數式”和“過程式”。在聲明式編程中，我們編寫的函數表示程序的 _邏輯_ _logic_，而不是 _流程_ _flow_。聲明式編程用於創建沒有 _副作用_ _side effects_ 的程序，這意味著在函數之外沒有狀態變化。聲明式編程語言包括Haskell，SQL和HTML等。相反，命令式編程就是程序員編寫一套程序的邏輯和流程結合在一起的程序。命令式編程語言包括例如BASIC，C，C++和Java。有些語言是“混合”的，這意味著他們鼓勵聲明式編程，但也可以用來表達一個必要的編程範式。這樣的混合體包括Lisp，Erlang，Prolog，JavaScript和Python。一般來說，任何命令式語言都可以用來在聲明式的範式中編寫，但它通常會導致不雅的代碼。相比之下，純粹的聲明式語言不能用來寫入一個命令式的範例。在純粹的聲明式語言中，_沒有“變量”_。
...
智能合約給程序員帶來了很大的負擔：錯誤會花費金錢。因此，編寫不會產生意想不到的影響的智能合約至關重要。要做到這一點，你必須能夠清楚地推斷程序的預期行為。因此，聲明式語言在智能合約中比在通用軟件中扮演更重要的角色。不過，正如你將在下面看到的那樣，最豐富的智能合約語言是命令式的（Solidity）。

智能合約的高級編程語言包括（按大概的年齡排序）：

LLL:: 一種函數式（聲明式）編程語言，具有類似Lisp的語法。這是以太坊智能合約的第一個高級語言，但今天很少使用。

Serpent:: 一種過程式（命令式）編程語言，其語法類似於Python。也可以用來編寫函數式（聲明式）代碼，儘管它並不完全沒有副作用。很少被使用。最早由Vitalik Buterin創建。

Solidity:: 具有類似於JavaScript，C ++或Java語法的過程式（命令式）編程語言。以太坊智能合約中最流行和最常用的語言。最初由Gavin Wood（本書的合著者）創作。

Vyper:: 最近開發的語言，類似於Serpent，並且具有類似Python的語法。旨在成為比Serpent更接近純粹函數式的類Python語言，但不能取代Serpent。最早由Vitalik Buterin創建。

Bamboo:: 一種新開發的語言，受Erlang影響，具有明確的狀態轉換並且沒有迭代流（循環）。旨在減少副作用並提高可審計性。非常新，很少使用。
...
Solidity是編寫智能合約的“面向合約的”編程語言。它用於在各種區塊鏈平臺上實施智能合約。它由Gavin Wood，Christian Reitwiessner，Alex Beregszaszi，Liana Husikyan，Yoichi Hirai和幾位以前的以太坊核心貢獻者開發，以便在區塊鏈平臺（如以太坊）上編寫智能合約。
...
要開始，我們將下載Solidity編譯器的二進制可執行文件。然後我們會編寫一個簡單的合約。
...
使用可以提高生產力的工具。最後，Solidity程序只是純文本文件。雖然花哨的編輯器和開發環境可以讓事情變得更容易，但除了簡單的文本編輯器（如vim（Linux / Unix），TextEdit（MacOS）甚至NotePad（Windows）），你無需任何其他東西。只需將程序源代碼保存為+.sol+擴展名即可，Solidity編譯器將其識別為Solidity程序。
...
我們使用+solc+的 +--bin+ 和 +--optimize+ 參數來生成我們示例合約的優化二進制文件：
...
+solc+產生的結果是一個可以提交給以太坊區塊鏈的十六進制序列化二進制文件。

[[eth_contract_abi_sec]]
=== 以太坊合約應用程序二進制接口（ABI）

在計算機軟件中，應用程序二進制接口（ABI）是兩個程序模塊之間的接口；通常，一個在機器代碼級別，另一個在用戶運行的程序級別。ABI定義瞭如何在*機器碼*中訪問數據結構和功能；不要與API混淆，API以高級的，通常是人類可讀的格式將訪問定義為*源代碼*。因此，ABI是將數據編碼到機器碼，和從機器碼解碼數據的主要方式。

在以太坊中，ABI用於編碼EVM的合約調用，並從交易中讀取數據。ABI的目的是定義合約中的哪些函數可以被調用，並描述函數如何接受參數並返回數據。

合約ABI的JSON格式由一系列函數描述（參見<<solidity_functions>>）和事件（參見<<solidity_events>>）的數組給出。函數描述是一個JSON對象，它包含`type`，`name`，`inputs`，`outputs`，`constant`和`payable`字段。事件描述對象具有`type`，`name`，`inputs`和`anonymous`的字段。
...
如你所見，編譯器會生成一個描述由 +Faucet.sol+ 定義的兩個函數的JSON對象。這個JSON對象可以被任何希望在部署時訪問 +Faucet+ 合約的應用程序使用。使用ABI，應用程序（如錢包或DApp瀏覽器）可以使用正確的參數和參數類型構造調用 +Faucet+ 中的函數的交易。例如，錢包會知道要調用函數+withdraw+，它必須提供名為 +withdraw_amount+ 的 +uint256+ 參數。錢包可以提示用戶提供該值，然後創建一個編碼它並執行+withdraw+功能的交易。
...
讓我們在我們的 +Faucet+ 合約中添加一條編譯指示。我們將命名新文件 +Faucet2.sol+，以便在我們繼續處理這些示例時跟蹤我們的更改：
...
boolean (bool):: 布爾值, +true+ 或 +false+, 以及邏輯操作符 pass:[! (not)], pass:[&& (and)], pass:[|| (or)], pass:[== (equal)], pass:[!= (not equal)].
...
地址:: 20字節的以太坊地址。+address+ 對象有 +balance+ (返回賬戶的餘額) 和 +transfer+ (轉移ether到該賬戶) 成員方法。
...
在EVM中執行合約時，它可以訪問一組較小範圍內的全局對象。這些包括 +block+，+msg+ 和 +tx+ 對象。另外，Solidity公開了許多EVM操作碼作為預定義的Solidity功能。在本節中，我們將檢查你可以從Solidity的智能合約中訪問的變量和函數。

===== 調用交易/消息上下文

msg:: +msg+對象是啟動合約執行的交易（源自EOA）或消息（源自合約）。它包含許多有用的屬性：
...
任何地址（作為輸入傳遞或從合約對象轉換而來）都有一些屬性和方法：
...
keccak256, sha256, sha3, ripemd160:: 用各種標準哈希算法計算哈希值的函數。

ecrecover:: 從簽名中恢復用於簽署消息的地址。

==== 合約的定義

Solidity的主要數據類型是_contract_對象，它在我們的+Faucet+示例的頂部定義。與面嚮對象語言中的任何對象類似，合約是一個包含數據和方法的容器。
...
public:: Public是默認的，這些函數可以被其他合約，EOA交易或合約內部調用。在我們的+Faucet+示例中，這兩個函數都被定義為public。
...
pure:: 純(pure)函數不讀寫任何變量。它只能對參數進行操作並返回數據，而不涉及任何存儲的數據。純函數旨在鼓勵沒有副作用或狀態的聲明式編程。
...
我們在<<intro>>中引入的+Faucet+示例合約沒有任何構造函數或自毀函數。這是永恆的合約，不能從區塊鏈中刪除。讓我們通過添加一個構造函數和selfdestruct函數來改變它。我們可能希望自毀僅由最初創建合約的EOA來調用。按照慣例，這通常存儲在稱為+owner+的地址變量中。我們的構造函數設置所有者變量，並且selfdestruct函數將首先檢查是否是所有者調用它。
...
如果其他人用 +owner+ 以外的地址調用 destroy 函數，則將失敗。但是，如果構造函數存儲在 +owner+ 中的地址調用它，合約將自毀，並將剩餘餘額發送到 +owner+ 地址。
...
在 <<function_modifier_onlyowner>> 中，我們看到函數修飾器的聲明，名為+onlyOwner+。此函數修飾器為其修飾的任何函數設置條件，要求存儲為合約的+owner+的地址與交易的+msg.sender+的地址相同。這是訪問控制的基本設計模式，只允許合約的所有者執行具有+onlyOwner+修飾器的任何函數。
...
Solidity的合約對象支持 _繼承_，這是一種用附加功能擴展基礎合約的機制。要使用繼承，請使用關鍵字+is+指定父合約：
...
像這樣的其他錯誤檢查代碼會略微增加gas消耗，但它比不檢查提供了更好的錯誤報告。在gas量和詳細錯誤檢查之間取得適當的平衡是你需要根據合約的預期用途來決定的。在為測試網絡設計的+Faucet+的情況下，即使額外報告成本更高，我們也不冒險犯錯。也許對於一個主網合約，我們會選擇節約gas用量。

==== 事件（Events）

事件是便於生產交易日誌的Solidity構造。當一個交易完成（成功與否）時，它會產生一個 _交易收據_ _transaction receipt_，就像我們在 <<evm>> 中看到的那樣。交易收據包含_log_條目，用於提供有關在執行交易期間發生的操作的信息。事件是用於構造這些日誌的Solidity高級對象。

事件在輕量級客戶端和DApps中特別有用，它可以“監視”特定事件並將其報告給用戶界面，或對應用程序的狀態進行更改以反映底層合約中的事件。

事件對象接收序列化的參數並記錄在區塊鏈的交易日誌中。你可以在參數之前應用關鍵字+indexed+，以使其值作為索引表（哈希表）的一部分，可以由應用程序搜索或過濾。
...
讓我們使用+truffle+在修訂的+Faucet+合約上運行測試交易。按照 <<truffle>> 中的說明設置項目目錄並編譯+Faucet+代碼。源代碼可以在本書的GitHub存儲庫中找到：
...
調用另一份合約最安全的方法是你自己創建其他合約。這樣，你就可以確定它的接口和行為。要做到這一點，你可以簡單地使用關鍵字+new+來實例化它，就像任何面向對象的語言一樣。在Solidity中，關鍵字+new+將在區塊鏈上創建合約並返回一個可用於引用它的對象。假設你想從另一個名為+Token+的合約中創建並調用+Faucet+合約：
...
這種合約建造機制確保你知道合約的確切類型及其接口。合約+Faucet+必須在+Token+範圍內定義，如果定義位於另一個文件中，你可以使用+import+語句來執行此操作：
...
在這裡，我們將地址作為參數提供給構造函數，並將其作為+Faucet+對象進行轉換。這比以前的機制風險大得多，因為我們實際上並不知道該地址是否實際上是+Faucet+對象。當我們調用+withdraw+時，我們假設它接受相同的參數並執行與我們的+Faucet+聲明相同的代碼，但我們無法確定。就我們所知，在這個地址的+withdraw+函數可以執行與我們所期望的完全不同的事情，即使它的命名相同。因此，使用作為輸入傳遞的地址並將它們轉換成特定的對象中比自己創建合約要危險得多。
...
函數中任何動態大小的數組循環，對每個元素執行操作或搜索特定元素會引入使用過多gas的風險。在找到所需結果之前，或在對每個元素採取行動之前，合約可能會用盡gas。
...
我們以開發模式啟動truffle，並執行一個JavaScript文件+gas_estimates.js+，其中包含：
...
代碼質量::
智能合約代碼是無情的。每個錯誤都可能導致經濟損失。你不應該像通用編程一樣對待智能合約編程。相反，你應該採用嚴謹的工程和軟件開發方法論，類似於航空航天工程或類似的不容樂觀的工程學科。一旦你“啟動”你的代碼，你就無法解決任何問題。
...
無論何時向合約發送付款（如本例中），接收方合約（A）都有機會執行_payable_函數，例如默認的fallback函數。但是，惡意攻擊者可以利用這種執行。想象一下，在A的payable fallback中，合約A_再次_調用B銀行的提款功能。B的提現功能現在將經歷重入，因為現在相同的初始交易正在引發循環調用。


"(1) A 調用 B  (2) B 調用 A 的 payable 函數 (1) A 再次調用 B "

在B的退出提現函數的第二次迭代中，B將再次檢查A是否有可用餘額。由於步驟3（其更新了A的餘額）尚未執行，所以對於B來說，無論該函數被重新調用多少次，A仍然具有可用資金來提現。只要有gas可以繼續運行，就可以重複該循環。當A檢測到gas量不足時，它可以在payable函數中停止呼叫B. B將最終執行步驟3，從A的餘額中扣除X. 然而，這時，B可能已經執行了數百次轉賬，並且只扣除了一次費用。在這次襲擊中，A有效地洗劫了B的資金。
...
任何編程範式的軟件開發人員通常都會遇到以行為，結構，交互和創建為主題的重複設計挑戰。通常這些問題可以概括並重新應用於未來類似性質的問題。當給定正式結構時，這些概括稱為*設計模式*。智能合約有自己的一系列重複出現的設計問題，可以使用下面描述的一些模式來解決。
...
也許最基本的軟件安全原則是最大限度地重用可信代碼。在區塊鏈技術中，這甚至會凝結成一句格言：“Do not roll your own crypto”。就智能合約而言，這意味著儘可能多地從經社區徹底審查的免費庫中獲益。

在Ethereum中，使用最廣泛的解決方案是https://openzeppelin.org/[OpenZeppelin]套件，從ERC20和ERC721的Token實現，到眾多眾包模型，到常見於“Ownable”，“Pausable”或“LimitBalance”等合約中的簡單行為。該存儲庫中的合約已經過廣泛的測試，並且在某些情況下甚至可以用作_de facto_標準實現。它們可以免費使用，並且由https://zeppelin.solutions[Zeppelin]和不斷增長的外部貢獻者列表構建和修復。
...
Embark Framework Testing:: Embark與Mocha集成，運行用JavaScript編寫的單元測試。這些測試使用在TestRPC/Ganache上部署的合約執行。Embark框架自動部署智能合約，並在合約被更改時自動重新部署它們。它還跟蹤已部署的合約，並在真正需要時部署合約。Embark包括一個測試庫，它可以在EVM中快速運行和測試你的合約，並使用assert.equal()等函數。Embark測試將在目錄測試下運行任何測試文件。
...
Populus:: Populus使用python和自己的鏈仿真器來運行用Solidity編寫的合約。單元測試是用pytest庫編寫的。Populus支持專門用於測試的書面合約。這些合約文件名應該與glob模式 ```Test*.sol``` 匹配，並且位於項目測試目錄```./tests/```下的任何位置。
...
....
獲取位於 +address+ 的存儲，並使用 +position+ 的偏移量顯示該合約中存儲的數據。
