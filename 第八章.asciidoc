...
智能合約通常以高階語言編寫，例如Solidity。但為了運行，必須將它們編譯為EVM中運行的低級字節碼（請參見 <<evm>>）。一旦編譯完成，它們就會隨著轉移到特殊的合約創建地址的交易被部署到以太坊區塊鏈中。每個合約都由以太坊地址標識，該地址源於作為發起賬戶和隨機數的函數的合約創建交易。合約的以太坊地址可以在交易中用作接收者，可將資金髮送到合約或調用合約的某個功能。
...
EVM是一臺虛擬計算機，運行一種特殊形式的 _機器程式碼_ ，稱為_EVM 字節碼_，就像你的計算機CPU運行機器程式碼x86_64一樣。我們將在 <<evm>> 中更詳細地檢查EVM的操作和語言。在本節中，我們將介紹如何編寫智能合約以在EVM上運行。

雖然可以直接在字節碼中編寫智能合約。EVM字節碼非常笨重，開發者難以閱讀和理解。相反，大多數以太坊開發人員使用高級符號語言編寫程序和編譯器，將它們轉換為字節碼。

雖然任何高階語言都可以用來編寫智能合約，但這是一項非常繁瑣的工作。智能合約在高度約束和簡約的執行環境（EVM）中運行，幾乎所有通常的用戶界面，操作系統界面和硬體界面都是缺失的。從頭開始構建一個簡約的智能合約語言要比限制通用語言並使其適用於編寫智能合約更容易。因此，為編程智能合約出現了一些專用語言。以太坊有幾種這樣的語言，以及產生EVM可執行字節碼所需的編譯器。

一般來說，程式語言可以分為兩種廣泛的編程範式：分別是聲明式和命令式，也分別稱為“函數式”和“過程式”。在聲明式編程中，我們編寫的函數表示程序的 _邏輯_ _logic_，而不是 _流程_ _flow_。聲明式編程用於創建沒有 _副作用_ _side effects_ 的程序，這意味著在函數之外沒有狀態變化。聲明式程式語言包括Haskell，SQL和HTML等。相反，命令式編程就是開發者編寫一套程序的邏輯和流程結合在一起的程序。命令式程式語言包括例如BASIC，C，C++和Java。有些語言是“混合”的，這意味著他們鼓勵聲明式編程，但也可以用來表達一個必要的編程範式。這樣的混合體包括Lisp，Erlang，Prolog，JavaScript和Python。一般來說，任何命令式語言都可以用來在聲明式的範式中編寫，但它通常會導致不雅的程式碼。相比之下，純粹的聲明式語言不能用來寫入一個命令式的範例。在純粹的聲明式語言中，_沒有“變量”_。

雖然命令式編程更易於編寫和讀取，並且開發者更常用，但編寫按預期方式 _準確_ 執行的程序可能非常困難。程序的任何部分改變狀態的能力使得很難推斷程序的執行，並引入許多意想不到的副作用和錯誤。相比之下，聲明式編程更難以編寫，但避免了副作用，使得更容易理解程序的行為。

智能合約給開發者帶來了很大的負擔：錯誤會花費金錢。因此，編寫不會產生意想不到的影響的智能合約至關重要。要做到這一點，你必須能夠清楚地推斷程序的預期行為。因此，聲明式語言在智能合約中比在通用軟體中扮演更重要的角色。不過，正如你將在下面看到的那樣，最豐富的智能合約語言是命令式的（Solidity）。
...
Solidity項目的主要“產品”是_Solidity Compiler（solc）_，它將用Solidity語言編寫的程序轉換為EVM字節碼，並生成其他製品，如應用程式二進制接口（ABI）。Solidity編譯器的每個版本都對應於並編譯Solidity語言的特定版本。
...
使用可以提高生產力的工具。最後，Solidity程序只是純文本檔案。雖然花哨的編輯器和開發環境可以讓事情變得更容易，但除了簡單的文本編輯器（如vim（Linux / Unix），TextEdit（MacOS）甚至NotePad（Windows）），你無需任何其他東西。只需將程序源程式碼保存為+.sol+擴展名即可，Solidity編譯器將其識別為Solidity程序。
...
在計算機軟體中，應用程式二進制接口（ABI）是兩個程序模塊之間的接口；通常，一個在機器程式碼級別，另一個在用戶運行的程序級別。ABI定義瞭如何在*機器碼*中訪問資料結構和功能；不要與API混淆，API以高級的，通常是人類可讀的格式將訪問定義為*源程式碼*。因此，ABI是將數據編碼到機器碼，和從機器碼解碼數據的主要方式。
...
在本節中，我們將看看Solidity語言的一些功能。正如我們在 <<intro>> 中提到的，我們的第一份合約示例非常簡單，並且在許多方面也存在缺陷。我們將逐漸改進這個例子，同時學習如何使用Solidity。然而，這不會是一個全面的Solidity教程，因為Solidity相當複雜且快速發展。我們將介紹基礎知識，併為你提供足夠的基礎，以便能夠自行探索其餘部分。Solidity的完整文件可以在以下網址找到：
...
函數修飾器是一個非常有用的工具，因為它們允許我們為函數編寫前提條件並一致地應用它們，使程式碼更易於閱讀，因此更易於審計安全問題。它們最常用於訪問控制，如示例中的“function_modifier_onlyowner”，但功能很多，可用於各種其他目的。
...
從Solidity v.0.4.22開始，+require+還可以包含有用的文本消息，可用於顯示錯誤的原因。錯誤消息記錄在交易日誌中。所以我們可以通過在我們的+require+函數中添加一條錯誤消息來改進我們的程式碼：
...
讓我們應用我們的天然氣估算函數來估計我們的+Faucet+示例的天然氣成本，使用此書程式碼庫中的程式碼：
...
在編寫智能合約時，安全是最重要的考慮因素之一。與其他程序一樣，智能合約將完全按寫入的內容執行，這並不總是開發者所期望的。此外，所有智能合約都是公開的，任何用戶都可以通過創建交易來與他們進行交互。任何漏洞都可以被利用，損失幾乎總是無法恢復。
...
可讀性/可審核性::
你的程式碼應易於理解和清晰。閱讀越容易，審計越容易。智能合約是公開的，因為任何人都可以對字節碼進行逆向工程。因此，你應該使用協作和開源方法在公開場合開發你的工作。你應該編寫文件良好，易於閱讀的程式碼，遵循作為以太坊社區一部分的樣式約定和命名約定。
...
DApp:: DApp使用本地Solidity程式碼（一個名為ds-test的庫）和一個Parity構建的Rust庫（稱為Ethrun）執行以太坊字節碼，然後斷言正確性。ds-test庫提供用於驗證控制檯中數據記錄的正確性和事件的斷言功能。
