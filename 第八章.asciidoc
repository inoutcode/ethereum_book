...
有許多方法可以用來下載和安裝Solidity，無論是作為二進制發行版還是從源代碼編譯。你可以在Solidity文件中找到詳細的說明：
...
在計算機軟體中，應用程式二進制接口（ABI）是兩個程序模塊之間的接口；通常，一個在機器代碼級別，另一個在用戶運行的程序級別。ABI定義瞭如何在*機器碼*中訪問數據結構和功能；不要與API混淆，API以高級的，通常是人類可讀的格式將訪問定義為*源代碼*。因此，ABI是將數據編碼到機器碼，和從機器碼解碼數據的主要方式。
...
可讀性/可審核性::
你的代碼應易於理解和清晰。閱讀越容易，審計越容易。智能合約是公開的，因為任何人都可以對字節碼進行逆向工程。因此，你應該使用協作和開源方法在公開場合開發你的工作。你應該編寫文件良好，易於閱讀的代碼，遵循作為以太坊社區一部分的樣式約定和命名約定。
