...
在計算機軟體中，應用程式二進制接口（ABI）是兩個程序模塊之間的接口；通常，一個在機器程式碼級別，另一個在用戶運行的程序級別。ABI定義瞭如何在*機器碼*中訪問數據結構和功能；不要與API混淆，API以高級的，通常是人類可讀的格式將訪問定義為*源程式碼*。因此，ABI是將數據編碼到機器碼，和從機器碼解碼數據的主要方式。
