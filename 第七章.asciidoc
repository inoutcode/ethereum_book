...
例如，你可能會注意到沒有表示發起人EOA的地址的“+from+”數據。EOA的公鑰可以很容易地從ECDSA簽名的+v,r,s+組成部分中派生出來。EOA的地址又可以很容易地從公鑰中派生出來。當你看到顯示“from”字段的交易時，是該交易所用的軟體添加了該字段。客戶端軟體經常添加到交易中的其他元數據包括塊編號（被挖掘之後生成）和交易ID（計算出的哈希）。同樣，這些數據來源於交易，但不是交易信息本身的一部分。
...
你的錢包將跟蹤其管理的每個地址的nonce。這很簡單，只要你只是從單一點發起交易即可。假設你正在編寫自己的錢包軟體或其他一些發起交易的應用程序。你如何跟蹤nonce？
